...
int str_count( char *buffer, const char *search ) {
	...
	if( counter < 0 ) {
		log_err( "str_count(): Integer overflow?" );
	}
...
char *str_append( char *buf1, long int size1, char *buf2, long int size2 ) {
	if( buf1 == NULL ) {
		log_err( "str_append(): buf1 broken" );
	}
	if( buf2 == NULL ) {
		log_err( "str_append(): buf2 broken" );
	}
	if( size1 < 0 ) {
		log_err( "str_append(): size1 broken" );
	}
	if( size2 < 0 ) {
		log_err( "str_append(): size2 broken" );
	}
	if( size2 == 0 ) {
		return buf1;
	}
	if( size1+size2 <= 0 ) {
		log_err( "str_append(): Overflow" );
	}
